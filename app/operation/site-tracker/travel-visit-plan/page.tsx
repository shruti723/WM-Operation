...
export default function TravelVisitPlanPage() {
    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/operation/site-tracker/travel-visit-plan?${params.toString()}`, {
                cache: "no-store",
            })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch("/api/operation/site-tracker/travel-visit-plan", {
                method: "POST",
                headers: {
                    ...
                    <div className="bg-orange-50 border-b border-orange-100 px-5 py-3 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                                Fill this form to submit a new travel or site visit plan.
                            </p>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-5">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    name="approvedByAmitoj"
                                    value={form.approvedByAmitoj}
                                    onChange={handleChange}
                                    className="w-full h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 bg-white"
                                >
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-slate-900 text-white">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs">Approved</th>
                                    <th className="px-4 py-3 text-left text-xs">Resolved</th>
                                ...
                                {records.length === 0 && !loading ? (
                                    ...
                                    records.map((record) => (
                                        <tr key={record.id} className="border-b border-slate-100 hover:bg-slate-50">
                                            ...
                                            <td className="px-4 py-3 text-slate-500 min-w-[260px]">{record.expectedOutcome || "-"}</td>
                                            ...
                                            <td className="px-4 py-3 text-right text-slate-700 whitespace-nowrap">
                                                {record.estimatedCost !== null && record.estimatedCost !== undefined
                                                    ? record.estimatedCost.toLocaleString("en-IN")
                                                    : "-"}
                                            </td>
                                            <td className="px-4 py-3 whitespace-nowrap">{yesNo(record.approvedByAmitoj)}</td>
                                            <td className="px-4 py-3 whitespace-nowrap">{yesNo(record.issueResolved)}</td>
